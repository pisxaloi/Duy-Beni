...
const renderBlock = `<div className="absolute inset-0 bg-zinc-900 group-hover:bg-zinc-800 transition-colors" />
                                <img
                                  src={\`/\${item.img}.png\`}
                                  className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity relative z-10"
                                  alt={item.title}
                                />`;

const newRenderBlock = `<div className="absolute inset-0 bg-zinc-900 group-hover:bg-zinc-800 transition-colors" />
                                {item.icon ? (
                                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-2">
                                    <item.icon className="w-8 h-8 text-white/40 group-hover:text-yellow-500 transition-colors z-10" />
                                    <span className="text-[9px] text-white/50 group-hover:text-yellow-500 font-bold uppercase tracking-widest">{item.title}</span>
                                  </div>
                                ) : (
                                  <img
                                    src={\`/\${item.img}.png\`}
...
c = c.replace(renderBlock, newRenderBlock);
