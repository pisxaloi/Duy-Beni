...
// generateLocalMessage fonksiyonunun başlangıcı - CRLF (\r\n) uyumlu
const funcStart = `/**\r
 * Deterministik seed kullanarak yerel (AI'siz) ilham mesajı üretir.\r
 * 10 kategori, 70+ eylem, 300+ hedef ve 200+ gerekçe ile\r
 * 100.000'den fazla benzersiz kombinasyon üretir, tekrar riskini minimize eder.\r
 */\r
function generateLocalMessage(\r
  seed: number,\r
  _unasTexts: string[],\r
  _jungTexts: string[],\r
  _skipCombinations: string[] = []\r
): {\r
  message: string;\r
} {`;

const funcEnd = `  return { message };\r
}`;
...
if (startIdx === -1 || endIdx === -1) {
  console.error('Fonksiyon bulunamadı! startIdx:', startIdx, 'endIdx:', endIdx);
  // Debug: fonksiyon başlangıcını ara
  const debugIdx = content.indexOf('function generateLocalMessage');
  console.error('function generateLocalMessage index:', debugIdx);
  if (debugIdx >= 0) {
    console.error('Çevreleyen metin:', JSON.stringify(content.substring(debugIdx, debugIdx + 200)));
  }
  process.exit(1);
}
...
const newFunction = `/**\r
 * Deterministik seed kullanarak yerel (AI'siz) ilham mesajı üretir.\r
 * Gemini önerisiyle yenilendi: Önceden yazılmış 32 derin mesaj havuzu kullanır.\r
 */\r
function generateLocalMessage(\r
  seed: number,\r
  _unasTexts: string[],\r
  ...
  // Gelişmiş seed'li rastgele sayı üreteci (deterministik)\r
  function seededRandom(s: number): () => number {\r
    let h = Math.abs(s) | 0;\r
    h = ((h ^ 0x9e3779b9) + (h << 6) + (h >> 2)) | 0;\r
    h = ((h ^ 0x85ebca6b) + (h << 3) + (h >> 5)) | 0;\r
    h = ((h ^ 0xc2b2ae35) + (h << 7) + (h >> 4)) | 0;\r
    let state = Math.abs(h) % 2147483647;\r
    if (state <= 0) state = 1;\r
    return () => {\r
      state = (state * 16807) % 2147483647;\r
      return (state - 1) / 2147483646;\r
    };\r
  }\r
\r
  const rand = seededRandom(seed);\r
\r
...
,    {
  ...
  ];  // Seed'e göre deterministik seçim\r
  const index = Math.abs(seed) % yeniMesajHavuzu.length;\r
  const seciliMesaj = yeniMesajHavuzu[index];\r
  const finalMessage = \`\${seciliMesaj.action}\n\n\${seciliMesaj.because}\n\nUnutma; \${seciliMesaj.touch}\`;\r
\r
  return { message: finalMessage };\r
}`;
